...
#define THISROWISNULL 1
    ...
    if (ptrbc->worthyOfPolynomialFitProcessing > POLYNOMIALDEGREE) {
        ...
        try {  //Attempt to read back the data we just inserted into table tbl_bucketized_by_tempf_ranges
            ...
            if (ptrbc->numberOfReturnedRows) { //Call this `if(ptrbc->numberOfReturnedRows)`
                ...
                for  (ptrbc->loopCounter = 0, ptrbc->rowBeingProcessed = 0; ptrbc->loopCounter < ptrbc->worthyOfPolynomialFitProcessing; ++ptrbc->loopCounter, ++ptrbc->rowBeingProcessed) { //Begin loading the gsl matrices and vectors with values.
                ...
                *ptrbc->outstring << ptrbc->squareRootOfSumOfTheSquaresOfTheVarience << "☜ Square Root Of Sum Of TheSquares Of The Varience; \n" << ptrbc->averageVariencePerValidReading << "☜ Average Varience Per Valid Reading\n" << ptrbc->averageVariencePerValidReading*100 << "☜ \% Average Varience Per Valid Reading\n"<< ptrbc->grc << "☜ Number of valid meter readings (# observed dependent variables)" <<  cases[ptrbc->debugFlags.intMyCase] << "\n";
                ...
                TblPolyFit *ptrTPF = new TblPolyFit(ptrbc->lookAtMyConnectionString, ptrbc->debugFlags.intMyCase);  //Make ptrTPF a constant pointer
                ...
                //                                          V                        V          V          V          V          V             V                                 V                       V
                if(ptrTPF->TblPolyFit::doInsertInto(ptrbc->lookAtMyConnectionString, &ptrTPF->pd, &ptrTPF->c0, &ptrTPF->c1 , &ptrTPF->c2, &ptrTPF->c3, (short *)ptrTPF->ptrMeterID , smf->getCorrelationCoefficient(), smf->getChi2() )) {
                    std::cout << "Some kind of problem in TblPolyFit::doInsertInto, got a return code of > 0 " << std::endl;
                } else {
                    std::cout << "Successful call to TblPolyFit::doInsertInto, got a return code of 0" <<  std::endl;
                }
//                if (ptrTPF != nullptr) delete ptrTPF;  //ABENDS here within a gsl routine, why? \
                After the ptrTPF destructor is called, the SetupForMultiFit destructor is called, WHY? \
                Perhaps it's because the SetupForMultiFit object is going `out-of-scope` at this point in the program?\
                I really dont what the SetupForMultiFit object to be destroyed at this point in the program! \
                Could it be that the C-based gsl does not know how to process constant pointer? \
                2018-06-20T07:40:38 ==> change TblPolyFit *const ptrTPF to just TblPolyFit *ptrTPF and let's see what happens.
                ; //Ok then don't do a delete ptrTPF, let the system call in it's destructors as we go out of scope.
            } else {
                // At this point, SetupForMultiFit destructor should have been called, too.
            ...
            } //END of `if(ptrbc->numberOfReturnedRows)`
        } catch (const std::exception &e) {
